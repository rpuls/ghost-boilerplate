    ...
    getAddress(preferred, options = { useFallbackAddress: false }) {
        if (preferred.replyTo && !this.#isValidEmailAddress(preferred.replyTo.address)) {
            ...
            if (preferred.replyTo.address.toLowerCase() !== this.defaultFromEmail.address.toLowerCase()) {
                ...
                logging_1.default.warn(`[EmailAddresses] replyTo address ${preferred.replyTo.address} matches the default from-address and failed generic validation; removing it`);
            }
        ...
        if (this.sendingDomain) {
            ...
            logging_1.default.warn(`[EmailAddresses] Cannot send emails from ${preferred.from.address} when sending domain is ${this.sendingDomain} — falling back to the default from-address`);
        }
