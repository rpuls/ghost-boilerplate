...
exports.CustomFieldDefinitionsService = void 0;
...
const FieldName = zod_1.z.string().trim().min(1, { message: 'Custom field name is required.' }).max(MAX_NAME_LENGTH, { message: 'Custom field name is too long.' });
...
const AddFieldsInput = zod_1.z.array(AddFieldInput)
    .min(1)
...
const EditFieldInput = zod_1.z.object({
    ...
    type: custom_field_types_1.FieldTypeSchema.optional()
});
class CustomFieldDefinitionsService {
    ...
    async read(key) {
        ...
        return zod_1.z.decode(codec_1.customFieldCodec, row);
    }
    /**
        ...
        try {
            created = await this.knex.transaction(async (trx) => {
                ...
                for (const [index, field] of fields.entries()) {
                    ...
                    await trx(TABLE).insert({ id: new bson_objectid_1.default().toHexString(), key, name: field.name, type: field.type, created_at: new Date() });
                    keys.push(key);
                }
            ...
            if (isUniqueConstraintViolation(err)) {
                throw new errors_1.default.ConflictError({ message: 'Could not create the custom field, please try again.' });
            }
            throw err;
        }
    ...
    async readMany(db, keys) {
        ...
        return keys.map(key => zod_1.z.decode(codec_1.customFieldCodec, byKey.get(key)));
    }
    /**
    ...
    async mintKey(db, base) {
        ...
        throw new errors_1.default.ValidationError({ message: 'Could not mint a unique key for this custom field.', property: 'name' });
    }
    /**
    ...
    async assertNameAvailable(db, name, exceptKey) {
        ...
        if (clash) {
            throw new errors_1.default.ValidationError({ message: 'A custom field with this name already exists.', property: 'name' });
        }
    }
    async edit(context, key, input) {
        const parsed = EditFieldInput.safeParse(input);
        if (!parsed.success) {
            throw new errors_1.default.ValidationError({ message: parsed.error.issues[0].message, property: parsed.error.issues[0].path[0]?.toString() });
        }
        ...
        if (patch.key !== undefined && patch.key !== existing.key) {
            throw new errors_1.default.ValidationError({ message: 'Custom field keys cannot be changed once created.', property: 'key' });
        }
        if (patch.type !== undefined && patch.type !== existing.type) {
            throw new errors_1.default.ValidationError({ message: 'Custom field types cannot be changed once created.', property: 'type' });
        }
        ...
        if (patch.name !== undefined && patch.name !== existing.name) {
            ...
            try {
                ...
                if (isUniqueConstraintViolation(err)) {
                    throw new errors_1.default.ConflictError({ message: 'Could not rename the custom field, please try again.' });
                }
                throw err;
            }
    ...
    async destroy(context, key) {
        ...
        if (field.status !== schema_1.FIELD_STATUS.archived) {
            throw new errors_1.default.ValidationError({ message: 'Only archived custom fields can be deleted. Archive the field first.' });
        }
...
exports.CustomFieldDefinitionsService = CustomFieldDefinitionsService;
...
function applyFilter(query, filter) {
    ...
    try {
        ...
        throw new errors_1.default.BadRequestError({ message: 'Could not parse the filter parameter.', property: 'filter', err: err });
    }
