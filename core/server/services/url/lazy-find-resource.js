...
function excludeFor(type) {
    const cfg = resourcesConfig.find((c) => c.type === type);
...
function pruneToEagerShape(record, type) {
...
function createFindResource(models) {
    const loadOne = async (Model, query, type, options = {}) => {
